...
// One id per loaded plugin instance: the liveness evidence that survives a
// host restart, mirroring the bootId the absent health route would have given.
const JINGXI_BOOT_ID = randomUUID()
...
let sessionCheckAt = 0
    ...
    if (webServer) {
      ...
      // DSH builds that do not serve /api/system/health still serve this
      // plugin's status route, which is the liveness fallback for the probe.
      const jingxiStatusUrl = buildJingxiStatusUrl(webServer.port) ?? null
      ...
      const updateVerification = webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const result = await getDshUpdateVerification({
            root: dshRoot, restartId, healthUrl: dshHealthUrl, healthFallbackUrl: jingxiStatusUrl,
          })
      ...
      const lifecycle = jingxiOpsEnabled ? webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const baseline = action === 'update'
            ? await readDshRuntimeHealth({ url: dshHealthUrl, fallbackUrl: jingxiStatusUrl })
            : undefined
