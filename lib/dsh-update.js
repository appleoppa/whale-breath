...
/**
 * Read a DSH manifest version from one candidate `package.json`.
 * @param file - path to one candidate manifest.
 * @returns the version when that manifest IS the DSH package, else undefined.
 */
async function readMemberDshVersion(file) {
  const manifest = await readJson(file)
  if (manifest?.name !== DSH_PACKAGE_NAME) return undefined
  return parseVersion(manifest.version) ? manifest.version : undefined
}

/**
 * Read the DSH version from a source checkout, where the published package is a
 * workspace member (`apps/*`) rather than a `node_modules` dependency. Such a
 * checkout has no `@deepseek-ai/dsh` entry in its root manifest, so the member
 * manifests are the only source of the version.
 * @param root - candidate checkout root.
 * @returns the member's version, else undefined.
 */
async function readWorkspaceDshVersion(root) {
  const manifest = await readJson(path.join(root, 'package.json'))
  // `workspaces` is JSON in package.json; pnpm-workspace.yaml is YAML, so test
  // for its presence rather than pretending to parse it.
  if (!Array.isArray(manifest?.workspaces) && !existsSync(path.join(root, 'pnpm-workspace.yaml'))) return undefined
  let entries
  try {
  ...
  for (const entry of entries) {
    ...
    const version = await readMemberDshVersion(path.join(root, 'apps', entry.name, 'package.json'))
...
/**
 * Whether a directory is a DSH install: an installed root that depends on the
 * DSH package, or a source checkout whose `apps/*` workspace builds it.
 * @param candidate - directory under test.
 * @returns whether the directory can serve as the DSH install root.
 */
async function isDshInstallRoot(candidate) {
  if (!candidate || !(await isDirectory(candidate))) return false
  if (!(await isDirectory(path.join(candidate, 'node_modules')))) return false
  const manifest = await readJson(path.join(candidate, 'package.json'))
  if (typeof manifest?.dependencies?.[DSH_PACKAGE_NAME] === 'string') return true
  return await readWorkspaceDshVersion(candidate) !== undefined
}
...
export async function readInstalledDshVersion(root) {
  ...
  if (parseVersion(declared)) return declared
  return readWorkspaceDshVersion(root)
}

/**
 * Read the host health contract without treating an HTTP response as proof.
 *
 * `url` is the optional `/api/system/health` route, which some DSH builds do
 * not serve. A 404 (absent route), or a 401/403 (the route exists but this
 * caller is not admitted to it), means the ROUTE is unusable — not that the
 * host is down — so `fallbackUrl` (this plugin's own status route) is probed
 * before reporting failure. Health is only ever claimed from a payload that
 * carries a boot id, so a reachable-but-unidentified host stays unverified and
 * `source` names the route that proved it.
 * @param options - fetch implementation, primary and fallback URLs, and timeout.
 * @returns the runtime health evidence, including which route proved it.
 */
export async function readDshRuntimeHealth({
  ...
  const probe = async (target) => {
    ...
    try {
      ...
      if (response?.ok === false || (Number.isFinite(status) && status > 0 && (status < 200 || status >= 300))) {
        return {
          ok: false,
          routeUnavailable: status === 404 || status === 401 || status === 403,
          state: 'runtime-unverified',
          error: 'DSH health endpoint unavailable',
        }
      }
      const payload = await readResponsePayload(response)
      if (typeof payload?.bootId !== 'string' || payload.bootId.trim() === '') {
        return { ok: false, state: 'runtime-unverified', error: 'DSH health response incomplete' }
      }
      // `/api/system/health` states readiness explicitly; the plugin's own status
      // route proves the host is serving, which is the fact this probe needs.
  ...
  const primary = typeof url === 'string' && url.trim() !== ''
    ? await probe(url)
    : { ok: false, routeUnavailable: true, state: 'runtime-unverified', error: 'DSH health endpoint unavailable' }
  if (primary.ok) return primary
  if (!primary.routeUnavailable || typeof fallbackUrl !== 'string' || fallbackUrl.trim() === '') return primary
  const fallback = await probe(fallbackUrl)
  if (fallback.ok) return { ...fallback, fallback: true }
  // Report the primary failure: the unusable contract route is what a reader
  // must fix, and the fallback error only says this plugin is not serving.
  return primary
}
...
/** Build this plugin's own status URL, the liveness fallback for a host without a health route. */
export function buildJingxiStatusUrl(port) {
  const numericPort = Number(port)
  if (!Number.isInteger(numericPort) || numericPort < 1 || numericPort > 65535) return undefined
  return `http://127.0.0.1:${numericPort}/api/jingxi/status`
}
...
export async function getDshUpdateVerification({
  ...
  const health = await readDshRuntimeHealth({ fetchImpl, url: healthUrl, fallbackUrl: healthFallbackUrl, timeoutMs })
