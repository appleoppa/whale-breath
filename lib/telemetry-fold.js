...
// ── 持久化 stream 镜像 ───────────────────────────────────────────────────
// DSH 不单独发 assistant/chunk 事件：增量块内嵌在 assistant/message 的
// data.stream 里（journal 视角的等价形态）。不解析它就永远拿不到首 token
// 到达时刻，firstTokenTime 恒为 undefined，速率/曲线一律 unavailable。
// 时间基准：stream 项自带的绝对时间与事件时间同一单调时钟域。
// 仅取时间锚点，不读取任何文本内容（授权红线：不存 raw content）。
function streamItemStartMs(item) {
  if (!item || typeof item !== 'object') return undefined
  if (Number.isFinite(item.time0)) return item.time0
  if (Number.isFinite(item.time)) return item.time
  return undefined
}
// dt 是相对 time0 的逐块毫秒增量。
function streamItemIsTokenDelta(item) {
  if (!item || typeof item !== 'object') return false
  if (item.type === 'text-chunks') return Array.isArray(item.texts) && item.texts.some(text => text !== '')
  if (item.type === 'reasoning-chunks') return Array.isArray(item.texts) && item.texts.some(text => text !== '')
  if (item.type === 'tool-call-chunks') return true
  return false
}
// 逐块时间锚点：只有流式块才带 dt 明细，block-start/block-end 只是标记。
function streamItemTickTimes(item) {
  const time0 = streamItemStartMs(item)
  if (time0 === undefined) return []
  if (!Array.isArray(item.dt) || item.dt.length === 0) return [time0]
  const times = [time0]
  let elapsed = 0
  for (const step of item.dt) {
    if (!Number.isFinite(step)) continue
    elapsed += step
    times.push(time0 + elapsed)
  }
  return times
}
// journal 形态的 usage 块与 live 形态同名同形，直接复用 onChunk 的分支。
function streamItemChunk(item) {
  return item && typeof item === 'object' && item.type === 'chunk' ? item.chunk : undefined
}
...
function onChunk(live, chunk, time, proxyTime = time) {
  if (live.turn === undefined) return
  ...
  const startMs = Math.floor(proxyTime / 250) * 250
  const last = live.proxyBins[live.proxyBins.length - 1]
  if (last !== undefined && last.startMs === startMs) { last.mass += 1; last.endMs = proxyTime } else { live.proxyBins.push({ startMs, endMs: proxyTime, mass: 1 }) }
}
// journal 形态：把 assistant/message.data.stream 逐项喂回与 assistant/chunk
// 相同的折叠路径。首 token 时刻取流式块 time0（而非整条消息落盘时刻），
// 曲线质量按每个 token 块的 dt 明细铺开，避免整轮塌成单点。
function onAssistantStream(live, stream, messageTime) {
  if (live.turn === undefined || !Array.isArray(stream)) return
  for (const item of stream) {
    const chunk = streamItemChunk(item)
    if (chunk !== undefined) { onChunk(live, chunk, messageTime); continue }
    if (!streamItemIsTokenDelta(item)) continue
    const ticks = streamItemTickTimes(item)
    for (const tick of ticks) onChunk(live, { type: 'text-delta', text: 'x' }, tick, tick)
  }
}
...
export function applyJingxiFold(state, event) {
  ...
  switch (event.type) {
    ...
    case 'assistant/message':
      // stream 必须先折叠：onAssistantMessage 依赖它写入的 firstTokenTime
      // 来结算本 step 的 TTFT 与速率窗口，顺序颠倒则一切速率指标仍为 unavailable。
      onAssistantStream(state.live, event?.data?.stream, event.time)
