...
本 fork 在保留上游完整历史的前提下，整合了 PGG 侧的两部分：

- **`pi/`** — Pi 原生等价实现（`pi/extensions/pgg-jingxi.ts` 扩展 + `pi/web/components/` 呼吸面板 + `pi/web/app/api/jingxi/` 只读接口），供 Pi agent 宿主使用。
- **`dsh/`** — DeepSeek Harness (DSH) 原插件 V1.0.2 的归档副本（原样保存，含 `lib/`、`assets/icons/`、`test/`）。

两者与上游 `lib/`（DSH 当前版本）并存，互不覆盖。
